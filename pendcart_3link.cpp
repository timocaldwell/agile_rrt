...
// Refer to header file for constructor details. Depending on the constructor, the control type is
//     type_:
//        kFREEDYNAMICS_TYPE
//        kFEEDFORWARD_TYPE
//        kTRACKTRAJECTORY_TYPE
//        kLINEARSTEERINGPROJECTION_TYPE
//        kLINEARSTEERINGPROJECTIONWCOST_TYPE
// The constructor specifies the type which determines how the control input uu is calculated in the overloaded operator().
...
PendCart::PendCart(const InterpVector & uu_ff_interp, const InterpVector & KK_interp, const InterpVector & xx_ref_interp, const constraints_struct & constraints, ode_state_type * uu_out)
...
PendCart::PendCart(const InterpVector & xxzero, const InterpVector & BB, const InterpVector & KKlin, const InterpVector & KKproj, const InterpVector & WWK, const InterpVector & Phi, const NSx1_type & eta, const NIxNI_type & RRinv, const constraints_struct & constraints, ode_state_type * uu_out)
...
PendCart::PendCart(const InterpVector & xxzero, const InterpVector & BB, const InterpVector & KKlin, const InterpVector & KKproj, const InterpVector & WWK, const InterpVector & Phi, const NSx1_type & eta, const NSxNS_type & QQ, const NIxNI_type & RR, const NIxNI_type & RRinv, const constraints_struct & constraints, ode_state_type * uu_out)
...
// Odeint calls the overloaded operator() to evaluate dxxdtt. The type_ determines the
// Hard coded linearization of ff(xx, uu). The equations were exported from Mathematica.
...
// Hardcoded dynamics of the triple pendulum on the cart ff(xx, uu).
// Repeated functions are computed once.
// state: xx = {theta1 , theta1_dot , theta2 , theta2_dot , theta3 , theta3_dot , X , X_dot}.
void PendCart::ff(const ode_state_type & xx, const vector<double> & uu, ode_state_type * dxxdtt) {
  double cx0 = cos(xx[0]);
  double sx0 = sin(xx[0]);
  double cx2 = cos(xx[2]);
  double sx2 = sin(xx[2]);
...
bool PendCart::IsConstraintSatisfied(const vector<double> & xx_prev, const vector<double> & xx_cur, const vector<double> & uu_cur) const {
  ...
  {
    if ( xx_cur[ii] < constraints_->xbounds( ii, 0 ) )
        return false;
    else if ( xx_cur[ii] > constraints_->xbounds( ii, 1 ) )
        return false;
  }
  for (int ii = 0; ii < NI; ++ii)
  {
    if ( uu_cur[ii] < constraints_->ubounds( ii, 0 ))
        return false;
    if ( uu_cur[ii] > constraints_->ubounds( ii, 1 ))
        return false;
  }
  ...
  for (int jj = 0; jj < numlinks_; ++jj) {
    ...
    { // is_constraintsatisfied == false when the line segment between the pendulum head of state xx (i.e. point rr) and xx_prev (i.e. point qq) breaks disk constraint of being obs_radii from point pp.
      double radius_squared = pow(constraints_->obs_radii[ii],2);
      double ppX = constraints_->obs_Xs[ii];
      double ppY = constraints_->obs_Ys[ii];
      if ( IsLineCircleIntersect(radius_squared, ppX, ppY, qqX, qqY, rrX, rrY) )
          return false;
    }
  }
  return true;
...
// Finds the point on the line segment nearest the circle center. Returns true if the distance from that point to the center is less than the circle radius.
// Note: radius_squared is the circle's radius squared.
bool PendCart::IsLineCircleIntersect(double radius_squared, double ppX, double ppY, double qqX, double qqY, double rrX, double rrY) const {
  double rrqq_distance_sqaured = ((rrX-qqX)*(rrX-qqX) + (rrY-qqY)*(rrY-qqY));
  double ss;
  if (rrqq_distance_sqaured == 0) // e.g. rr and qq are the same point so choose one.
    ss = 0;
  else
    ss = ((ppX-qqX)*(rrX-qqX) + (ppY-qqY)*(rrY-qqY)) / rrqq_distance_sqaured;
  if (ss <= 0.0 && pow(qqX-ppX,2) + pow(qqY-ppY,2) < radius_squared)
    return true;
  if (ss >= 1.0 && pow(rrX-ppX,2) + pow(rrY-ppY,2) < radius_squared)
    return true;
  double llX = (rrX - qqX)*ss + qqX;
  double llY = (rrY - qqY)*ss + qqY;
  if (ss >= 0.0 && ss <= 1.0 && pow(llX-ppX,2) + pow(llY-ppY,2) < radius_squared)
    return true;
  return false;
}
