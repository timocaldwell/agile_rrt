...
private:
    ...
    double linklen = 0.333333;

    vector<double> uu;
    size_t type;
    InterpVector * uu_ff_interp; // feedforward inputs
    InterpVector * KK_interp; // Gains
    InterpVector * xx_ref_interp; // Trajectory tracking

    InterpVector *xxzero, *BB, *KKlin, *KKproj, *WWK, *Phi;
    NSx1_type eta;
    NIxNI_type RRinv;
    NSxNS_type QQ;
    NIxNI_type RR;

    ode_state_type xx_ref_pt;
    
    kin_constraints constraints;
    
    ode_state_type * uu_out;
    
    bool is_linecircleintersect(double, double, double, double, double, double, double) const;
    
public:
    PendCart () : type(FREEDYNAMICS_TYPE), uu(NI,0.0) {}
    PendCart (InterpVector * uu_ff_interp_in) : uu_ff_interp(uu_ff_interp_in) , type(FEEDFORWARD_TYPE) {}
    PendCart (InterpVector * KK_interp_in, const ode_state_type & xx_ref_pt_in) : type(TRACKPOINT_TYPE) , KK_interp(KK_interp_in), xx_ref_pt(xx_ref_pt_in) {}
    PendCart (InterpVector * uu_ff_interp_in, InterpVector * KK_interp_in, InterpVector * xx_ref_interp_in) :  uu_ff_interp(uu_ff_interp_in), KK_interp(KK_interp_in), xx_ref_interp(xx_ref_interp_in), type(TRACKTRAJECTORY_TYPE) {}
    PendCart (ode_state_type * uu_out_in, InterpVector * uu_ff_interp_in, InterpVector * KK_interp_in, InterpVector * xx_ref_interp_in, const kin_constraints & constraints_in) :  uu_out(uu_out_in), uu_ff_interp(uu_ff_interp_in), KK_interp(KK_interp_in), xx_ref_interp(xx_ref_interp_in), constraints(constraints_in), type(TRACKTRAJECTORY_TYPE) {}
    PendCart (ode_state_type * uu_out_in, InterpVector *xxzero_in, InterpVector *BB_in, InterpVector *KKlin_in, InterpVector *KKproj_in, InterpVector *WWK_in, InterpVector *Phi_in, const NSx1_type &eta_in, const NIxNI_type &RRinv_in, const kin_constraints & constraints_in) :  uu_out(uu_out_in), xxzero(xxzero_in), BB(BB_in), KKlin(KKlin_in), KKproj(KKproj_in), WWK(WWK_in), Phi(Phi_in), eta(eta_in), RRinv(RRinv_in), constraints(constraints_in), type(LINEARSTEERINGPROJECTION_TYPE) {}
    PendCart (ode_state_type * uu_out_in, InterpVector *xxzero_in, InterpVector *BB_in, InterpVector *KKlin_in, InterpVector *KKproj_in, InterpVector *WWK_in, InterpVector *Phi_in, const NSx1_type &eta_in, const NSxNS_type &QQ_in, const NIxNI_type &RR_in, const NIxNI_type &RRinv_in, const kin_constraints & constraints_in) :  uu_out(uu_out_in), xxzero(xxzero_in), BB(BB_in), KKlin(KKlin_in), KKproj(KKproj_in), WWK(WWK_in), Phi(Phi_in), eta(eta_in), QQ(QQ_in), RR(RR_in), RRinv(RRinv_in), constraints(constraints_in), type(LINEARSTEERINGPROJECTIONWCOST_TYPE) {}
 
    void operator()( const ode_state_type xx , ode_state_type &dxxdtt , const double tt );
 
    template<typename TT>
    void calc_AA( ode_state_type xx, vector<double> uu, Eigen::MatrixBase<TT> & AA );
//    void calc_AA(ode_state_type xx, vector<double> uu, NSxNS_type & AA);
    
    template<typename TT>
    void calc_BB( ode_state_type xx, vector<double> uu, Eigen::MatrixBase<TT> & BB );
//    void calc_BB(ode_state_type xx, vector<double> uu, NSxNI_type & BB);

    bool is_constraintsatisfied( vector<double> &, vector<double> &, vector<double> & ) const;
...
template<typename TT>
void PendCart::calc_AA( ode_state_type xx, vector<double> uu, Eigen::MatrixBase<TT> & AA ) {
...
template<typename TT>
void PendCart::calc_BB( ode_state_type xx, vector<double> uu, Eigen::MatrixBase<TT> & BB ) {
...
#endif /* defined(__Agile_RRT__pendcart__) */
