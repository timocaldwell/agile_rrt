...
void Tree::PrintTree(treenode_ & node, double dt, const string & name, ostream & stream) {
...
void Tree::PrintBranch(treenode_ & node, double dt, const string & name, ostream & stream) {
  node.vert->PrintEdgeTraj(dt, name, stream);
  if (node.parent)
...
void Tree::NearestNeighbor(treenode_ & node, treenode_ ** best_node_ptr, double & best_dist, const NSx1_type & xxsamp, const double delta, const double t_h) {
...
Tree::Tree (TreeVertex * init_vert, double t_h_max_upper, sampling_bnds bnds, int max_cnt, int max_miss, int max_samplemiss, int printskip, double seed)
    : t_h_max_upper_(t_h_max_upper), bnds_(bnds), max_cnt_(max_cnt), max_miss_(max_miss), max_samplemiss_(max_samplemiss), computedisttogoal_(false), printskip_(printskip), disttogoal_(kLARGENUM), cnt_(0), miss_(0), samplemiss_(0), isstopdist_(false), stopdist_(kLARGENUM), nndelta_(kLARGENUM), seed_(seed) {
  root_ = new treenode_;
...
void Tree::RunRRT() {
  ...
  default_random_engine rand_engine((double)cur_time * seed_); // TO SEED TO THE TIME
...
//        samplemiss = 0;
  ...
  while (cnt_ < max_cnt_ && miss_ < max_miss_ && samplemiss_ < max_samplemiss_) {
    ...
    {
      uniform_real_distribution<double> unif(bnds_.xbnds(ii, 0), bnds_.xbnds(ii, 1));
    ...
    NearestNeighbor(*root_, &best_node, best_dist, xxsamp, nndelta_, t_h);
